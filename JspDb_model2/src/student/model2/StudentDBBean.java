...
public class StudentDBBean {
	...
	public StudentDBBean(){	//생성자. 리스트 객체를 생성함. 클래스가 싱글턴으로 설계되었기 때문에 멤버변수인 리스트도 공유된다.
	...
	public int createTable() {//createDB.jsp에서 호출함. 테이블을 생성함.
		...
		closeConnection();	//이거 위치를 호출한 곳에 둘까 여기 둘까? Exception이 발생하면 실행되지 않지만 자바는 가비지 콜렉터가 있으니 문제 없으려나...?
		return isError;
	}
	...
	public int singleInsert(String name, int kor,int eng,int mat) throws Exception{	//insertDB.jsp에서 호출함. 레코드 하나를 추가하고 학번을 반환함.
	...
	public List<StudentDataBean> getStudentList(){		//AllviewDB.jsp에서 호출함. examtable의 모든 레코드를 성적순으로 가져온다음 리스트에 적재함.
		...
		getConnection();
		try {
			rset = stmt.executeQuery("SELECT name,studentid,kor,eng,mat,total,average, "+
	...
	public StudentDataBean getStudentInfo(int studentid){//학번을 기준으로 학생정보를 검색하고 찾았을 경우 StudentDataBean 객체를 반환함. 못찾으며 null을 반환함.
