...
<body>
<h1 align=center>레코드 수정(페이지네이션)</h1>
<%!
...
<%
		...
		//처음으로
		out.println("<a href='http://192.168.23.14:8080/JspDb/DB01/updateDB_paged.jsp?from=0&cnt="+cntPT+"&SID="+SID+"'>처음으로</a>");
		//<<버튼
		out.println("<a href='http://192.168.23.14:8080/JspDb/DB01/updateDB_paged.jsp?from="+(int)(Math.floor(prevBtn()*10/100))*100+"&cnt="+cntPT+"&SID="+SID+"'><<</a>");
		if(studentList.size()==1){
			out.println("<a href='http://192.168.23.14:8080/JspDb/DB01updateDB_paged.jsp?from=0&cnt="+cntPT+"&SID="+SID+"'><b>1</b></a>");
		}
		...
		for(int i=startPg;i<endPg;i++){
			...
			if(i==curPg){
				out.println("<a href='http://192.168.23.14:8080/JspDb/DB01/updateDB_paged.jsp?from="+i*10+"&cnt="+cntPT+"&SID="+SID+"' ><b>"+(i+1)+"</b></a>");	
			}else{
				out.println("<a href='http://192.168.23.14:8080/JspDb/DB01/updateDB_paged.jsp?from="+i*10+"&cnt="+cntPT+"&SID="+SID+"'>"+(i+1)+"</a>");					
			}

		}
		//>> 버튼
		out.println("<a href='http://192.168.23.14:8080/JspDb/DB01/updateDB_paged.jsp?from="+(int)(Math.floor(nextBtn()*10/100))*100+"&cnt="+cntPT+"&SID="+SID+"'>>></a>");
		//마지막으로 버튼
		out.println("<a href='http://192.168.23.14:8080/JspDb/DB01/updateDB_paged.jsp?from="+((totalPg*10)-cntPT)+"&cnt="+cntPT+"&SID="+SID+"'>마지막으로</a>");
		out.println("</div>");
	}
